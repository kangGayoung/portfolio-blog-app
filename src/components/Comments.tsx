...
export default function Comments({post, getPost}: CommentsProps){
    //console.log(post?.comments?.slice(0)?.reverse()); 댓글 정렬 바꾸기
    const [comment, setComment] = useState("");
    const {user} = useContext(AuthContext); //유저정보 가져오기
        ...
        try{
            if(post && post?.id){
                ...
                await updateDoc(postRef, {
                    ...
                    updateDated:new Date()?.toLocaleDateString("ko",{ //게시글 작성시간 순서 정렬
                        hour:"2-digit",
                        minute:"2-digit",
                        second:"2-digit", 
                        }),    
                    });
                    // 댓글 생성 후 코멘트 리스트 업데이트
                    await getPost(post.id);
                }
            }
            toast.success("댓글 생성했습니다.");
            setComment(""); //코멘트 초기화
        } catch (e: any){
            console.log(e);
            toast.error(e?.code);
